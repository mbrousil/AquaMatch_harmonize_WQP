...
These `characteristicNames` are chosen in order to select for only those measurements that pertain to CDOM.
...
We next ensure that the media type for all CDOM data is `"Surface Water"`, `"Water"`, `"Estuary"`, or `NA`. `r step_4_dropped` rows are removed. The final row count after this is `r step_4_rows`.
...
The next step in CDOM harmonization is converting the units of WQP records. Records that don't make sense for CDOM, are not in mg per liter (mg/L) or relative fluorescence units (RFU), or can't be converted to mg/L are dropped. We use the unit conversion table for this. Note that both RFU and mg/L are retained for the final data product, but treated separately throughout the harmonization process.
...
We next review the analytical methods used in measuring CDOM by classifying the text provided with each record in `ResultAnalyticalMethod.MethodName`, `USGSPCode`, and `ResultSampleFractionText`. Once these methods are classified we arrange them into hierarchical tiers as described in the *Method tiering* section of the *Tiering, flagging, and quality control philosophy* chapter.

However, we found that there were methods for fluorescent Dissolved Organic Matter (fDOM) in addition to CDOM. So, prior to classification we assign "fDOM" or "CDOM" to each record in the `parameter` column based on the `USGSPCode` column. [`USGSPCode` values](https://help.waterdata.usgs.gov/parameter_cd?group_cd=%) of 32295, 32322, and 32330 are assigned "fDOM", while the rest are assigned "CDOM". (Note that we refer to records in this dataset as "CDOM" despite both of these types of data being present).

In preparation for tiering, we check the `ResultAnalyticalMethod.MethodName` column for names that indicate non-CDOM measurements. The only phrase used to flag and remove unrelated methods from CDOM data is "9222", which we determined indicated a non-CDOM method.
...
|---|---|---|---|
| 0 | Restrictive | Data that are verifiably self-similar across organizations and  time-periods and can be considered highly reliable and interoperable | ResultSampleFractionText column is "Dissolved" and ResultAnalyticalMethod.MethodName mentions wavelength or wavelength is implied by the USGSPCode column |
| 1 | Narrowed | Data that we have good reason to believe are self-similar, but for which  we can’t verify full interoperability across data providers | Fluorometer mentioned in ResultAnalyticalMethod.MethodName, USGSPCode is NA, and ResultSampleFractionText is "Dissolved"; OR ResultAnalyticalMethod.MethodName mentions the wavelength but ResultSampleFractionText is "Total"; OR ResultAnalyticalMethod.MethodName is NA and USGSPCode doesn't imply wavelength |
| 2 | Inclusive | Data that are assumed to be reliable and are harmonized to our best  ability given the information available from the data provider. This  tier includes  NA or non-resolvable descriptions for the  analytical method, which often make up the majority of methods descriptions for any given parameter | ResultAnalyticalMethod.MethodName is "Colorized Dissolved Organic Matter" but USGSPCode and ResultSampleFractionText are NA; OR ResultAnalyticalMethod.MethodName, USGSPCode, and ResultSampleFractionText are all NA |

<br>

At this point we export a file (`3_harmonize/out/cdom_tiering_record.csv`) that contains a record of how specific method text was tiered and how many row counts corresponded to each method.
...
This will be filled with `NA` for CDOM. Some parameters will have additional flagging requirements that CDOM does not, so we include this placeholder to maintain the same columns across all parameter data products.
...
Before finalizing the dataset we remove CDOM values that are beyond a realistic threshold. We use 10,000 mg/L as our cutoff for removal. This is based on values from Gray et al. (2000), *"Comparability of suspended-sediment concentration and total suspended solids data"*. SSC values in SSC-CDOM pairs in the Gray et al. dataset were below 10,000 with one exception and most CDOM values were lower than their paired SSC value.
...
The final step of CDOM harmonization is to aggregate simultaneous observations. Prior to aggregation, the dataset has the following distribution of values when organized by `CharacteristicName` and `ProviderName`:

<br>

<img src="3_harmonize/out/cdom_charname_dists.png" alt="Distribution of CDOM values by CharacteristicName and ProviderName" width="650"/>
...
<img src="3_harmonize/out/cdom_tier_hex_map.png" alt="Geographic distribution of SSC/CDOM records by tier in the conterminous US" width="650"/>
...
<img src="3_harmonize/out/cdom_tier_top_depth_dist_postagg.png" alt="Distribution of harmonized_top_depth_value column values by parameter, tier, and ResolvedMonitoringLocationTypeName" width="650"/>
