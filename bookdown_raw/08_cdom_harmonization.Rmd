...
**NOTE**: These characteristicNames will be further filtered down to only include "Absorbance at 280 nanometers" later on in this document for the purpose of simplifying this workflow during development.

<br>

### Temporary step to select only a single `CharacteristicName`

**NOTE**: This is a temporary section while this bookdown doc is being developed specifically for the CDOM dataset. Before the next harmonization step all `CharacteristicName` values **except** for "Absorbance at 280 nanometers" are removed to allow for barebones testing of the harmonization framework. 

Around 110K rows are lost because of this filtering step, which below this **appear** to be dropped during unit harmonization. This is just a note that they are **not** dropped because of units.
...
We next ensure that the media type for all CDOM data is `"Surface Water"`, `"Water"`, `"Estuary"`, or `NA`. 

**NOTE**: At this step we also remove records with the `CharacteristicName` "Beam Attenuation (Seabird)", which accounts for a large number of rows but which is not relevant to the dataset.
...
The next step in CDOM harmonization is converting the units of WQP records. Records that don't make sense for CDOM or can't be converted to AU/m dropped. We use the unit conversion table below for this. 
...
We next review the analytical methods used in measuring CDOM alongside relevant information in `CharacteristicName` and `USGSPCode`. We use the information in these columns to construct hierarchical tiers as described in the *Method tiering* section of the *Tiering, flagging, and quality control philosophy* chapter.
...
|---|---|---|---|
| 0 | Restrictive | Data that are verifiably self-similar across organizations and  time-periods and can be considered highly reliable and interoperable | Has wavelength listed in methods columns or USGSPCode, and if there's a wavelength in the CharacteristicName then it matches that in methods/USGSPCode. |
| 1 | Narrowed | Data that we have good reason to believe are self-similar, but for which  we can’t verify full interoperability across data providers | Wavelengths included in multiple columns, but different columns may not agree. |
| 2 | Inclusive | Data that are assumed to be reliable and are harmonized to our best  ability given the information available from the data provider. This  tier includes  NA or non-resolvable descriptions for the  analytical method, which often make up the majority of methods descriptions for any given parameter | All other instances. |
<br>
...
Before finalizing the dataset we remove CDOM values that are beyond a realistic threshold. This step does not currently have a threshold value for CDOM measurements set, so it's more of a placeholder at this point. 

However, we also check for and remove any depths > 592m, the deepest point in a lake in the U.S.
...
1. Histograms showing the distribution of harmonized measurements (top) and CVs (bottom) broken down by `tier` ***after*** aggregating simultaneous records. **However,** at this point in the dataset's development there is only a single tier of data present (tier 0, restrictive). Additionally, there are very few simultaneous records in the dataset, so only a few CVs are produced.
