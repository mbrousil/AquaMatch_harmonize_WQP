...
harmonize_cdom <- function(raw_cdom, p_codes){
  ...
  walk2(.x = fail_counts,
        .y = names(fail_counts),
        .f = ~ ggsave(filename = paste0("3_harmonize/out/cdom_",
                                        .y,
                                        "_fail_pie.png"),
  ...
  print(
    paste0(
      ...
      nrow(cdom) - nrow(cdom_fails_removed)
    )
  )
  ...
  dropped_fails <- tibble(
    ...
    number_dropped = nrow(cdom) - nrow(cdom_fails_removed),
    n_rows = nrow(cdom_fails_removed),
    order = 2)
  ...
  mdl_updates <- cdom_fails_removed %>%
  ...
  dropped_mdls <- tibble(
    ...
    number_dropped = nrow(cdom_fails_removed) - nrow(cdom_mdls_added),
    n_rows = nrow(cdom_mdls_added),
    order = 3
  )
  ...
  cdom_approx$approx_value <- as.numeric(str_replace_all(cdom_approx$ResultMeasureValue_original, c("\\*" = "")))
  cdom_approx$approx_value[is.nan(cdom_approx$approx_value)] <- NA
  
  # Keep important data
  cdom_approx <- cdom_approx %>%
    select(approx_value, index)
  
  print(
    paste(
      round((nrow(cdom_approx)) / nrow(cdom_mdls_added) * 100, 3),
      "% of samples had values listed as approximated"
    )
  )
  
  # Replace harmonized_value field with these new values
  cdom_approx_added <- cdom_mdls_added %>%
    left_join(x = ., y = cdom_approx, by = "index") %>%
    mutate(harmonized_value = ifelse(index %in% cdom_approx$index,
                                     approx_value,
                                     harmonized_value),
           # Flag: 1 = used approximate adjustment, 0 = value not adjusted
           approx_flag = ifelse(index %in% cdom_approx$index, 1, 0))
  
  dropped_approximates <- tibble(
    step = "cdom harmonization",
    reason = "Dropped rows while cleaning approximate values",
    short_reason = "Clean approximates",
    number_dropped = nrow(cdom_mdls_added) - nrow(cdom_approx_added),
    n_rows = nrow(cdom_approx_added),
    order = 4
  )
  ...
  greater_vals <- cdom_approx_added %>%
    # First, remove the samples that we've already approximated:
    filter((!index %in% mdl_updates$index) & (!index %in% cdom_approx$index)) %>%
    # Then select fields where the NUMERIC value column is NA....
    filter(is.na(ResultMeasureValue) & 
             # ... AND the original value column has numeric characters...
             grepl("[0-9]", ResultMeasureValue_original) &
  ...
  print(
    paste(
      round((nrow(greater_vals)) / nrow(cdom_approx_added) * 100, 9),
      "% of samples had values listed as being above a detection limit//greater than"
    )
  )
  
  # Replace harmonized_value field with these new values
  cdom_harmonized_values <- cdom_approx_added %>%
  ...
  dropped_greater_than <- tibble(
    ...
    number_dropped = nrow(cdom_approx_added) - nrow(cdom_harmonized_values),
    n_rows = nrow(cdom_harmonized_values),
    order = 5
  )
  ...
  dropped_na <- tibble(
    ...
    number_dropped = nrow(cdom_harmonized_values) - nrow(cdom_no_na),
    n_rows = nrow(cdom_no_na),
    order = 6
  )
  ...
  print(
    paste0(
      ...
      nrow(tiered_methods_cdom) - nrow(field_flagged_cdom)
    )
  )
  ...
  dropped_field <- tibble(
    ...
    number_dropped = nrow(tiered_methods_cdom) - nrow(field_flagged_cdom),
    n_rows = nrow(field_flagged_cdom),
    order = 10
  )
  ...
  dropped_misc <- tibble(
    ...
    number_dropped = nrow(field_flagged_cdom) - nrow(misc_flagged_cdom),
    n_rows = nrow(misc_flagged_cdom),
    order = 11
  )
  ...
  dropped_unreal <- tibble(
    ...
    number_dropped = nrow(misc_flagged_cdom) - nrow(realistic_cdom),
  ...
  # 1. Harmonized values
  no_simul_cdom_tier_label <- no_simul_cdom %>%
    mutate(
  ...
  print(
    paste0(
      ...
      nrow(realistic_cdom) - nrow(no_simul_cdom)
    )
  )
  ...
  dropped_simul <- tibble(
    ...
    number_dropped = nrow(realistic_cdom) - nrow(no_simul_cdom),
    n_rows = nrow(no_simul_cdom),
    order = 13
  )
  ...
  documented_drops_out_path <- "3_harmonize/out/cdom_harmonize_dropped_metadata.csv"
  ...
  data_out_path <- "3_harmonize/out/cdom_harmonized_final.csv"
  
  write_csv(no_simul_cdom,
            data_out_path)
