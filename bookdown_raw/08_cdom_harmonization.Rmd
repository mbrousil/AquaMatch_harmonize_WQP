...
We then assign each record to one of the `parameter` values in the table below, based on its values for `CharacteristicName`, `ResultAnalyticalMethod.MethodName`, and `ResultMeasure.MeasureUnitCode`. This helps us to group comparable measurements throughout the rest of the process:
...
|---|---|
| Absorbance at 254 nm | `CharacteristicName` is either "Specific UV Absorbance at 254 nm" or "UV 254" and `ResultMeasure.MeasureUnitCode` is **not** "L/mg-cm" or "L/mgDOC*m" |
...
| Absorbance at 440 nm | `CharacteristicName` is "Absorbance at 440 nanometers" OR `CharacteristicName` is "Colored dissolved organic matter (CDOM)" and `ResultAnalyticalMethod.MethodName` is "CDOM absorption (440nm)" |
| FDOM | `CharacteristicName` is "Colored dissolved organic matter (CDOM)" AND `ResultAnalyticalMethod.MethodName` is NA or mentions a fluorometer AND `ResultMeasure.MeasureUnitCode` is one of "RFU", "ug/l QSE", "mg/l", or "ug/L" |
| SUVA | `ResultMeasure.MeasureUnitCode` is "L/mg-cm" or "L/mgDOC*m" |

<br> 

**Temporary filter**:
Following the `parameter` assignments we currently drop records that **do not** match the wavelengths 254, 280, 370, or 440 in the `parameter` column. This is a temporary step to simplify the data harmonization process while this workflow is in development. Around 58K rows are lost because of this filtering step. The figure below makes it **appear** that these are dropped because of the media filtering step, but this is not the case. 
