...
p3_cdom_targets_list <- list(
  ...
  tar_target(
    name = p3_wqp_data_aoi_formatted_cdom,
    command = format_columns(p2_wqp_data_aoi_cdom),
    format = "feather"
  ),
  
  # Join in column containing site type info
  tar_target(
    name = p3_wqp_data_aoi_sitetype_cdom,
    command = left_join(
      x = p3_wqp_data_aoi_formatted_cdom,
      y = p1_wqp_inventory_aoi_cdom %>%
  ...
  tar_target(
    name = p3_wqp_data_aoi_date_time_cdom,
    command = fill_date_time(dataset = p3_wqp_data_aoi_sitetype_cdom,
                             site_data = p2_site_counts_cdom),
    packages = c("tidyverse", "lutz", "sf", "sfheaders")
  ),
  
  tar_target(
    name = p3_wqp_data_aoi_ready_cdom,
    command = clean_wqp_data(wqp_data = p3_wqp_data_aoi_date_time_cdom,
                             char_names_crosswalk = p1_char_names_crosswalk_cdom,
                             site_data = p2_site_counts_cdom,
                             wqp_metadata = p1_wqp_inventory_aoi_cdom),
    packages = c("tidyverse", "feather")
  ),
  
  # Connect cleaned data output to the pipeline
  tar_file_read(
    name = p3_cleaned_wqp_data_cdom,
    command = p3_wqp_data_aoi_ready_cdom$wqp_data_clean_path,
  ...
  tar_target(
    name = p3_cdom_harmonized,
    command = harmonize_cdom(raw_cdom = p3_cleaned_wqp_data_cdom,
                             p_codes = p3_p_codes),
  ...
  # Harmonized CDOM data containing grouping IDs for simultaneous
  # records, but not aggregated
  tar_file_read(
    name = p3_cdom_preagg_grouped,
    command = p3_cdom_harmonized$cdom_grouped_preagg_path,
    read = read_feather(path = !!.x),
    packages = "feather"),
  
  # Harmonized CDOM data after simultaneous record aggregation (i.e.,
  # final product)
  tar_file_read(
    name = p3_cdom_agg_harmonized,
    command = p3_cdom_harmonized$cdom_harmonized_path,
    read = read_csv(file = !!.x)),
  
  # Create a copy of the csv in feather format
  tar_file_read(
    name = p3_cdom_agg_harmonized_feather,
    command = {
      out_path <- gsub(x = p3_cdom_harmonized$cdom_harmonized_path,
                       pattern = ".csv",
                       replacement = ".feather")
      
      write_feather(x = p3_cdom_agg_harmonized,
                    path = out_path)
  ...
  # Export
  tar_target(
    name = p3_cdom_agg_harmonized_feather_drive_file,
    command = {
      p0_check_cdom_drive
      export_single_file(target = p3_cdom_agg_harmonized_feather,
                         drive_path = p0_cdom_output_path,
                         stable = p0_harmonization_config$cdom_use_stable,
                         google_email = p0_harmonization_config$google_email,
                         date_stamp = p0_harmonization_config$cdom_stable_date)
    },
  ...
  tar_file_read(
    name = p3_cdom_harmonized_site_info,
    command = {
      # Pull and clean data
      cdom_sites <- get_site_info(dataset = p3_cdom_preagg_grouped)
      
      out_path <- "3_harmonize/out/cdom_harmonized_site_info.feather"
  ...
  # Export
  tar_target(
    name = p3_cdom_site_info_drive_file,
    command = {
      p0_check_cdom_drive
      export_single_file(target = p3_cdom_harmonized_site_info,
                         drive_path = p0_cdom_output_path,
                         stable = p0_harmonization_config$cdom_use_stable,
                         google_email = p0_harmonization_config$google_email,
                         date_stamp = p0_harmonization_config$cdom_stable_date)
