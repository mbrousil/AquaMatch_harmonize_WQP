...
# Water Color harmonization process

```{r echo=FALSE}
documented_drops <- params$documented_drops %>%
  filter(grepl(pattern = "water_color", x = step, ignore.case = TRUE),
         !((step == "water_color harmonization") & short_reason == "Start"))
wc_chars <- params$wc_chars
...
The text below first walks through the pre-harmonization steps for the Water Color dataset and then delves into the specifics of the harmonization process.
...
## Pre-harmonization of the raw Water Color WQP dataset
...
At the start of the pre-harmonization process the raw Water Color WQP dataset contains `r step_1_rows` rows.
...
## Harmonization-ready Water Color dataset

Once ready for harmonization, the Water Color WQP dataset contains the following user-defined `CharacteristicNames`: `r wc_chars`.

<img src="3_harmonize/out/wc_stacked_char_names.png" alt="Stacked bar chart with counts of CharacteristicName records" width="650"/>

These `CharacteristicNames` are chosen in order to select for only those measurements that pertain to Water Color. We selected them by reviewing options in the `Characteristics` field of the [Water Quality Portal Filter Results](https://www.waterqualitydata.us/#advanced=true) page. 
...
We next ensure that the media type for all Water Color data is `"Surface Water"`, `"Water"`, `"Estuary"`, or `NA`. Any records not meeting these criteria are dropped.
...
```{r fig.alt="Table containing Water Color summary information for records that were changed because of their USGSPCode value."}
param_change_table %>%
  kable() %>%
  kable_paper() %>%
...
<img src="3_harmonize/out/wc_stacked_param_names.png" alt="Stacked bar chart with counts of parameter records" width="650"/>
...
<img src="3_harmonize/out/wc_ActivityCommentText_fail_pie.png" alt="ActivityCommentText failure detects" width="650"/>

#### ResultCommentText fail detects

<img src="3_harmonize/out/wc_ResultCommentText_fail_pie.png" alt="ResultCommentText failure detects" width="650"/>

#### ResultLaboratoryCommentText fail detects

<img src="3_harmonize/out/wc_ResultLaboratoryCommentText_fail_pie.png" alt="ResultLaboratoryCommentText failure detects" width="650"/>

<br>

#### ResultDetectionConditionText fail detects

<img src="3_harmonize/out/wc_ResultDetectionConditionText_fail_pie.png" alt="ResultDetectionConditionText failure detects" width="650"/>

#### ResultMeasureValue_original fail detects

<img src="3_harmonize/out/wc_ResultMeasureValue_original_fail_pie.png" alt="ResultMeasureValue_original failure detects" width="650"/>
...
The next step in Water Color harmonization is standardizing the units of WQP records. Records that don't make sense for Water Color or can't be converted to a standardized unit are dropped. We use the unit conversion table below to filter the units. "PCU", "CU", and "pt" units are standardized as "PCU". "None" and "ADMI value" are left as-is.

**Note** that while we retain both "PCU" and "ADMI value" here they are **not** interconvertible.

```{r echo=FALSE, message = FALSE}
read_csv("../3_harmonize/out/wc_unit_table.csv") %>%
...
<img src="3_harmonize/out/wc_unit_drop_pie.png" alt="ResultMeasure.MeasureUnitCode mismatched codes" width="650"/>
...
In preparation for tiering, we check the `ResultAnalyticalMethod.MethodName` column for names that indicate non-Water Color measurements and also remove rows where `ResultSampleFractionText` was "Acid Soluble". 
...
|---|---|---|---|
| 0 | Restrictive | Data that are verifiably self-similar across organizations and  time-periods and can be considered highly reliable and interoperable | For Apparent Color: Fraction is "Total", "Non-Filterable", "Non-Filterable (Particle)", "None", "Suspended", or "Unfiltered". Units are not "None". Methods use spectrophotometry (SM 2120-C, EPA 110.3, Hach 8025). For True Color: Fraction is "Dissolved", "Filterable", "Filtered, field", or "Filtered, lab". Same unit and methods rules as Apparent Color. |
| 1 | Narrowed | Data that we have good reason to believe are self-similar, but for which  we can’t verify full interoperability across data providers | For Apparent Color: Same as in Tier 0, but visual methods are used (SM 2120-B, EPA 110.2, mention of "visual", or "CC003".) For True Color: Same as in Tier 0, but visual methods are used as with Apparent Color in this Tier. |
| 2 | Inclusive | Data that are assumed to be reliable and are harmonized to our best  ability given the information available from the data provider. This tier includes NA or non-resolvable descriptions for the  analytical method, which often make up the majority of methods  descriptions for any given parameter | Samples for True Color that use non-standard wavelengths (345, 440nm). All other records not already categorized into Tier 0 or Tier 1 above. |
<br>

At this point we create a file (`3_harmonize/out/wc_tiering_record.csv`) that contains a record of how specific method text was tiered and how many row counts corresponded to each method. This file is also included as an interactive HTML table below, in order to illustrate how the specifics of each tiering decision in the table above are carried out. 

**Note**, for example, that this table contains a few instances where Tier 2 data contain mismatches between records' stated methods and their units: e.g., `ResultAnalyticalMethod.MethodName` = "Color by Calculating ADMI Values" but `ResultMeasure.MeasureUnitCode` = "PCU" and `ResultAnalyticalMethod.MethodName` = "Color Analysis Using Platinum/Cobalt" but `ResultMeasure.MeasureUnitCode` = "ADMI value". Records like this make up an exceedingly small component of the dataset.

```{r fig.alt="Table containing combinations of Water Color record summary information and corresponding tiers."}
...
Some parameters in published AquaMatch products will have different flagging requirements for this column that Water Color does not. Please keep this in mind and review the documentation for other harmonized WQP data products to ensure that the `field_flag` column is used as intended for each product.

No records should be removed by this process and so there are `r step_13_dropped` rows dropped leaving `r step_13_rows` remaining in the harmonized Water Color dataset.
...
Next we add a flag column, `misc_flag`, which indicates whether the value in the `harmonized_value` column exceeds 200 PCU. We use this as an indication for whether or not a value is within the typically observed range for natural waters. The value is based on the 99.5th percentile True Color measurements (n = 3,152; max = 350 PCU) recorded from the combined 2007, 2012, 2017, and 2022 National Lakes Assessment surveys ("NLA"; U.S. Environmental Protection Agency 2010, 2016, 2022, 2024).

The `misc_flag` column uses the following definition:

+ `misc_flag` = `0` when the color measurement has PCU for `harmonized_units` and the `harmonized_value` is less than 200
+ `misc_flag` = `1` when the color measurement has PCU for `harmonized_units` and the `harmonized_value` is 200 or greater
+ `misc_flag` = `2` when the color measurement is in non-PCU units and so can't be compared to the NLA measures

**Note** that while the cutoff for this flag was chosen based on the NLA, results over 200 PCU (flag = 1) are not necessarily erroneous. They are just above a common threshold.

Some parameters in published AquaMatch products will have different flagging requirements for this column that Water Color does not. Please keep this in mind and review the documentation for other harmonized WQP data products to ensure that the `misc_flag` column is used as intended for each product.

<br>

No records should be removed by this process and so there are `r step_14_dropped` rows dropped leaving `r step_14_rows` remaining in the harmonized Water Color dataset.
...
The final step of Water Color harmonization is to aggregate simultaneous observations.
...
`r step_16_dropped` rows dropped leaving `r step_16_rows` remaining in the final harmonized and aggregated Water Color dataset.
...
### Harmonized Water Color

At this point the harmonization of the Water Color data from the WQP is complete and we export the final dataset for use later in the workflow.
...
<img src="3_harmonize/out/wc_unit_dists_location_postagg.png" alt="Distribution of Water Color values by location type" width="650"/>
...
<img src="3_harmonize/out/wc_tier_dists_postagg.png" alt="Distribution of Water Color values by tier" width="650"/>


<img src="3_harmonize/out/wc_tier_cv_dists_postagg.png" alt="Distribution of Water Color CVs by tier" width="650"/>

<br>

3. Maps showing the geographic distribution of records by tier within the US:

<img src="3_harmonize/out/wc_tier_hex_map.png" alt="Geographic distribution of Water Color records by tier in the conterminous US" width="650"/>

<br>

4. Bar charts showing the distribution of values by tier across years, months, and days of the week:

<img src="3_harmonize/out/wc_year_tier_chart.png" alt="Temporal distribution of Water Color records by tier across years" width="650"/>

<img src="3_harmonize/out/wc_month_tier_chart.png" alt="Temporal distribution of Water Color records by tier across months" width="650"/>

<img src="3_harmonize/out/wc_wday_tier_chart.png" alt="Temporal distribution of Water Color records by tier across days of the week" width="650"/>

<br>

5. Lastly, bar charts showing the distribution of depth values by parameter, location type, and tier:

<img src="3_harmonize/out/wc_tier_top_depth_dist_postagg.png" alt="Distribution of harmonized_top_top_value column values by parameter, tier, and ResolvedMonitoringLocationTypeName" width="650"/>

<img src="3_harmonize/out/wc_tier_bottom_depth_dist_postagg.png" alt="Distribution of harmonized_top_bottom_value column values by parameter, tier, and ResolvedMonitoringLocationTypeName" width="650"/>

<img src="3_harmonize/out/wc_tier_discrete_depth_dist_postagg.png" alt="Distribution of harmonized_discrete_depth_value column values by parameter, tier, and ResolvedMonitoringLocationTypeName" width="650"/>
