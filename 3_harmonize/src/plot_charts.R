...
#' @param n_cols Custom input string to provide to ggplot2::facet_wrap(ncol), if facet_wrap
#' is used. Defaults to 2.
plot_time_charts <- function(dataset, param_name,
                             custom_width = 6, custom_height = 4,
                             scale_type = "fixed", year_seq = 10,
                             legend_position = "right", n_cols = 2){
  ...
  year_plot <- plot_data %>%
    ...
    xlab("Activity year") +
    ggtitle(
      paste0("Record distribution by year & tier: ", param_name)
    ) +
  ...
  # If more than a single value for parameter column...
  if(length(unique_params) > 1){
    year_plot <- year_plot +
      facet_wrap(vars(parameter), scales = scale_type, ncol = n_cols)
  }
  ...
  day_plot <- plot_data %>%
    ...
    xlab("Activity day") +
    ggtitle(
      paste0("Record distribution by day of week & tier: ", param_name)
    ) +
    scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
    scale_fill_viridis_d("Tier", direction = -1) +
    theme_bw() +
  ...
  ggsave(filename = paste0("3_harmonize/out/", param_name, "_",
                           "wday_tier_chart.png"),
  ...
  month_plot <- plot_data %>%
    ...
    xlab("Activity month") +
    ggtitle(
      paste0("Record distribution by month & tier: ", param_name)
    ) +
    scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
    scale_fill_viridis_d("Tier", direction = -1) +
    theme_bw() +
